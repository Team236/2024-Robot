...
public class ModOneWooferShot extends ParallelCommandGroup {
  ...
  public ModOneWooferShot(Intake intake, Cartridge cartridge, Tilt tilt, Drive drive, Elevator elevator) {
    ...
    new PIDCartridgeMotors(cartridge, Constants.CartridgeShooter.WOOFER_PID_LEFT_RPM, Constants.CartridgeShooter.WOOFER_PID_RIGHT_RPM).withTimeout(5)  //run cart motors in parallel with every command in Auto
    );
  }
}