...
//****** ModWooferRight CALLS CARTRIDGE MOTORS IN PARALLEL - SO NO CART MOTORS USED HERE *********/
    ...
    Commands.parallel(
      new PIDDrive(drive, Constants.DriveConstants.PULL_AWAY_TO_NOTE).withTimeout(2.3),
      new IntakeWithCounter(intake, Constants.Intake.INTAKE_SPEED).withTimeout(2.3),
      new PIDCartridgeTilt(tilt, Constants.Tilt.TILT_ENC_REVS_PODIUM).withTimeout(2.3)  
      ),
      Commands.parallel(
        new PIDDrive(drive, -Constants.DriveConstants.PULL_AWAY_TO_NOTE+3).withTimeout(1.5),
    ...
    Commands.parallel(
      ...
      new PIDTurnCCW(drive, Constants.DriveConstants.TURN_SIDE_OF_WOOFER+4).withTimeout(1.5)
      ),
